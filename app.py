...
def Welcome():
    return (
        ...
        f"The format for the last two routes is YYYY-MM-DD"
    )
...
def tobs():
    ...
    results = session.query(Measurement.tobs).filter(
        Measurement.date > date_calc).all()
...
def by_date(start=None,end=None):
    ...
    if not end:
        ...
        temps_all = list(np.ravel(results))
...
#     results = session.query(func.max(Measurement.tobs), func.min(
#         Measurement.tobs), func.avg(Measurement.tobs)).filter(Measurement.date >= start).filter(Measurement.date <= end).all()
